...
def main():
    ...
    daily_service.collect_daily_disclosures(
        target_company_names=company_names, start_date=bgn_de, end_date=end_de
    )
    ...
    try:
        logger.info(
            "💾 SQLite 최신 영속성 스냅샷 기반으로 최종 엑셀을 단방향 갱신(Export)합니다."
        )
        ...
        success_export = export_service.export_integrated_financial_data(
            str(output_file)
        )
            ...
            if drive_folder_id:
                try:
                    ...
                    if not os.path.exists(token_path):
                        logger.warning(
                            f"구글 드라이브 토큰 파일이 존재하지 않아 업로드를 생략합니다: {token_path}"
                        )
                    else:
                        ...
                        if output_file.exists():
                            ...
                            if success:
                                logger.info(
                                    "✨ 구글 드라이브 업로드 완료 (재무제표.xlsx)"
                                )
                            else:
                                logger.error("❌ 구글 드라이브 업로드 실패")
                        else:
                            logger.warning(
                                f"업로드할 로컬 엑셀 파일이 존재하지 않습니다: {output_file}"
                            )
                except Exception as drive_err:
                    logger.error(
                        f"구글 드라이브 업로드 연동 중 오류 발생: {drive_err}",
                        exc_info=True,
                    )
        else:
            logger.warning(
                "통합 엑셀 생성에 실패하여 구글 드라이브 업로드 및 동기화를 보류합니다."
            )
