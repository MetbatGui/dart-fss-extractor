    ...
    if target_file.exists():
        try:
            ...
            if target_col is not None:
                ...
                logger.info(
                    f"'{args.companies}'의 '{target_col}' 컬럼을 사용하여 {len(company_names)}개 기업을 로드했습니다."
                )
            else:
                ...
                if df.shape[1] >= 2:
                    ...
                    if first_val.isdigit() and len(first_val) == 6:
                        ...
                        logger.info(
                            f"'{args.companies}'의 첫 번째 열(코드 형태)을 건너뛰고, 두 번째 열을 사용하여 {len(company_names)}개 기업을 로드했습니다."
                        )
                ...
                if not company_names:
                    ...
                    logger.info(
                        f"'{args.companies}'의 첫 번째 열을 사용하여 {len(company_names)}개 기업을 로드했습니다."
                    )
        except Exception as e:
