...
REVENUE_TAGS = {
    ...
    "operatingrevenue",
    "salesrevenuenet",
    "operatingrevenueoffinancialservices",
    "revenuefinancialservices",
    "revenuefrominterest",
...
NI_TAGS = {"profitloss", "netincome", "profitlossfromcontinuingoperations"}
    ...
    def parse_xbrl_zip(
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, "r") as z:
                ...
                lab_ko_file = next(
                    (f for f in filenames if f.endswith("_lab-ko.xml")), None
                )

                if not xbrl_file:
                    logger.error(
                        f"[{corp_name}] ZIP 파일 내에 .xbrl 파일이 존재하지 않습니다."
                    )
                    return results
        ...
        for want_consolidated in [True, False]:
            ...
            for ctx in contexts.values():
                if (
                    ctx["type"] == "duration"
                    and ctx.get("is_pure", True)
                    and ctx["is_consolidated"] == want_consolidated
                ):
                    try:
            ...
            for elem in root.iter():
                ...
                if ko_label:
                    ...
                    if not is_rev and ko_label_clean in [
                        "매출액",
                        "영업수익",
                        "매출액(매출총이익)",
                    ]:
                        is_rev = True
                    if not is_op and ko_label_clean in [
                        "영업이익",
                        "영업손실",
                        "영업이익(손실)",
                        "영업손익",
                ...
                if (
                    not ctx.get("is_pure", True)
                    or ctx["is_consolidated"] != want_consolidated
                ):
                    continue
            ...
            for ctx in contexts.values():
                if (
                    ...
                    try:
                        ...
                        if e_d == max_end_date:
                            ctx_months = (
                                (e_d.year - s_d.year) * 12 + (e_d.month - s_d.month) + 1
                            )
                            if ctx_months == target_cum_months:
                                start_date_c = s_d
                                break
            ...
            for loc in root.findall(".//link:loc", ns):
                ...
                if loc_id and concept_name:
                    concept_clean = re.sub(
                        r"^(entity\d+|dge\d+|dar\d+)_", "", concept_name
                    )
