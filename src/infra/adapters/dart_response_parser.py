...
class DartResponseParser:
    ...
    def parse_financial_statement(
        ...
        all_results = DartResponseParser.parse_all(
            response_data, corp_code, year, report_type
        )
    ...
    @staticmethod
    def _parse_accounts(
        items: list[dict[str, Any]], fs_type: FinancialStatementType
    ) -> list[AccountItem]:
        ...
        return [
            ...
            if (
                not item.get("fs_div")
                or str(item.get("fs_div")).strip() == ""
                or item.get("fs_div") == target_div
            )
        ]

    @staticmethod
    def _parse_date_info(
        items: list[dict[str, Any]], report_type: ReportType
        ...
        if not thstrm_dt:
            ...
            return (
                None,
                None,
                (report_type == ReportType.ANNUAL or has_cumulative_keyword),
            )
