...
def _fiscal_period_no(stmt: FinancialStatement | None) -> int | None:
...
def _same_fiscal_period(
    a: FinancialStatement | None, b: FinancialStatement | None
) -> bool:
...
class FinancialMetrics:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            or self.operating_profit is not None
            or self.net_income is not None
        )
    ...
    def subtract(self, other: "FinancialMetrics") -> "FinancialMetrics":
        ...
        return FinancialMetrics(
            ...
            operating_profit=(
                Amount(self.operating_profit) - Amount(other.operating_profit)
            ).value,
    ...
    def sanitize(self, label: str, corp_name: str) -> "FinancialMetrics":
        ...
        if self.revenue is not None and self.revenue < 0:
            logger.warning(
                f"[{corp_name} {label}] 매출액 음수 감지 ({self.revenue}). None으로 치환합니다."
            )
            return FinancialMetrics(
                revenue=None,
...
class QuarterlyMetrics:
    ...
    annual_metrics: FinancialMetrics | None = None

    @classmethod
    def calculate_from_statements(
        cls,
        corp_name: str,
        q1_stmt: FinancialStatement | None,
        semi_stmt: FinancialStatement | None,
        q3_stmt: FinancialStatement | None,
        annual_stmt: FinancialStatement | None,
        revenue_kws: list[str],
        op_profit_kws: list[str],
        net_income_kws: list[str],
        target_fs_type: FinancialStatementType | None = None,
    ) -> "QuarterlyMetrics":
        ...
        def check_has_cumulative(stmt: FinancialStatement | None) -> bool:
            if not stmt:
                return False
        ...
        # CFS/OFS 여부 동적 체크
        has_cfs_by_report = {
            "1Q": q1_stmt.fs_type == FinancialStatementType.CONSOLIDATED
            if q1_stmt
            else False,
            "2Q": semi_stmt.fs_type == FinancialStatementType.CONSOLIDATED
            if semi_stmt
            else False,
            "3Q": q3_stmt.fs_type == FinancialStatementType.CONSOLIDATED
            if q3_stmt
            else False,
            "Annual": annual_stmt.fs_type == FinancialStatementType.CONSOLIDATED
            if annual_stmt
            else False,
        }
        ...
        def extract(
            ...
            if target_fs_type and stmt.fs_type != target_fs_type:
                if (
                    target_fs_type == FinancialStatementType.CONSOLIDATED
                    and stmt.fs_type == FinancialStatementType.SEPARATE
                ):
                    if not has_cfs_by_report.get(report_key, False):
                        logger.info(
                            f"[{stmt.corp_name} {report_key}] CFS 공시가 없어 OFS를 Fallback 수용합니다."
                        )
                    else:
                        return FinancialMetrics()
                else:
            ...
            return FinancialMetrics(
                ...
                operating_profit=stmt.find_account_amount(
                    op_profit_kws, use_cumulative
                ).value,
                net_income=stmt.find_account_amount(
                    net_income_kws, use_cumulative
                ).value,
            )
        ...
        def resolve_cumulative(
            stmt_cum: FinancialMetrics,
            stmt_single: FinancialMetrics,
            fallback_cum: FinancialMetrics,
            has_cum: bool,
        ) -> FinancialMetrics:
            if not has_cum:
                return fallback_cum
            if stmt_cum.is_valid:
                # 가짜 누적 검사
                if (
                    stmt_cum.revenue == stmt_single.revenue
                    and stmt_cum.operating_profit == stmt_single.operating_profit
                    and stmt_cum.net_income == stmt_single.net_income
                ):
                    return fallback_cum
                return stmt_cum
            return fallback_cum
        ...
        is_fake_2q = (
            has_q2_cum_flag
            and q1_final_cum.revenue is not None
            and q1_final_cum.revenue > 0
            and semi_single.revenue is not None
            and semi_single.revenue == semi_cum.revenue
            and semi_single.revenue > q1_final_cum.revenue
        )
        if is_fake_2q:
            logger.info(
                f"[{corp_name} 2Q] 가짜 단독 공시 오염 감지 -> 누적으로 치환하여 차감 복원합니다."
            )
            semi_cum = semi_single
            semi_single = FinancialMetrics()

        q2_final_cum = resolve_cumulative(
            semi_cum, semi_single, q1_final_cum.add(semi_single), has_q2_cum_flag
        )

        # 3분기 가짜 단독 공시 오염 가드 (이전 누계 대조)
        is_fake_3q = (
            has_q3_cum_flag
            and q2_final_cum.revenue is not None
            and q2_final_cum.revenue > 0
            and q3_single.revenue is not None
            and q3_single.revenue == q3_cum.revenue
            and q3_single.revenue > q2_final_cum.revenue
        )
        if is_fake_3q:
            logger.info(
                f"[{corp_name} 3Q] 가짜 단독 공시 오염 감지 -> 누적으로 치환하여 차감 복원합니다."
            )
            q3_cum = q3_single
            q3_single = FinancialMetrics()

        q3_final_cum = resolve_cumulative(
            q3_cum, q3_single, q2_final_cum.add(q3_single), has_q3_cum_flag
        )
        ...
        if semi_stmt is not None:
            q2_final_single = (
                semi_single
                if semi_single.is_valid
                else q2_final_cum.subtract(q1_final_cum)
            )

        q3_final_single = FinancialMetrics()
        if q3_stmt is not None:
            q3_final_single = (
                q3_single if q3_single.is_valid else q3_final_cum.subtract(q2_final_cum)
            )
