...
class DailyCollectionService:
    ...
    def __init__(
        ...
        download_port: DownloadPort | None = None,
    ...
    def collect_daily_disclosures(
        ...
        """
        logger.info(
            f"📅 데일리 공시 스캔 시작: {start_date} ~ {end_date} (대상 기업 수: {len(target_company_names)})"
        )
        ...
        corp_code_to_name: dict[str, str] = {
            c: n for n, c in zip(target_company_names, codes) if c
        }
        ...
        disclosures = self._financial_port.get_disclosures(
            bgn_de=start_date, end_de=end_date, pblntf_ty="A"
        )
        ...
        for disc in disclosures:
            ...
            if rcept_no in cache_dict:
                ...
                logger.info(
                    f"  💡 [캐시 히트] [{corp_name}] {report_nm} ({rcept_no}) - 이미 최근 3일 이내에 수집된 기록이 존재하여 스킵합니다. (만료일시: {valid_cache.get('expired_at')})"
                )
                success_codes.append(corp_code)
                continue

            logger.info(
                f"🎯 대상 기업 공시 포착: [{corp_name}] {report_nm} (접수번호: {rcept_no})"
            )
            ...
            if not period_info:
                logger.warning(
                    f"  ❌ 공시 제목에서 결산 연월을 추출하지 못했습니다: {report_nm}"
                )
                continue
            ...
            logger.info(
                f"  📊 판별 정보: 대상 연도={target_year}, 분기={target_quarter}, 정정공시 여부={is_amendment}"
            )
            ...
            if success:
                ...
                cache_updated = True
                logger.info(
                    f"  [메모리 캐시 갱신] {corp_name} / {rcept_no} (만료예정: {expired_time.isoformat()})"
                )
            else:
        ...
        if cache_updated and self._cache_port is not None:
            try:
                ...
                logger.info(
                    "💾 갱신된 수집 캐시 딕셔너리를 디스크(JSON 파일)에 일괄 영속화 완료했습니다."
                )
            except Exception as e:
    ...
    def _process_single_disclosure(
        ...
        try:
            ...
            if not is_amendment and year in company.success_years:
                logger.info(
                    f"  💡 [{corp_name}] {year}년 데이터가 이미 성공적으로 저장되어 있어 스킵합니다."
                )
                return True
            ...
            if self._xbrl_collector_port:
                ...
                if zip_file_path.is_file():
                    logger.info(
                        f"  💾 [로컬 캐시 히트] 원본 XBRL ZIP 파일 발견: {zip_file_path.name}"
                    )
                    zip_data = zip_file_path.read_bytes()
                elif self._download_port:
                    logger.info(
                        f"  📡 [다운로드 시도] XBRL ZIP 다운로드 시작 (접수번호: {rcept_no})"
                    )
                    try:
                        fetched_data = self._download_port.download_xbrl_zip(rcept_no)
                        if fetched_data and isinstance(
                            fetched_data, (bytes, bytearray)
                        ):
                            zip_data = fetched_data
                            zip_file_path.write_bytes(zip_data)
                            logger.info(
                                f"  💾 [다운로드 성공] 원본 ZIP 파일 캐시 저장 완료: {zip_file_path.name}"
                            )
                    except Exception as down_err:
                        logger.warning(
                            f"  ⚠️ 원본 ZIP 다운로드 또는 로컬 쓰기 중 경고: {down_err}"
                        )
                ...
                if zip_data and self._xbrl_parser_port:
                    try:
                        ...
                        logger.error(
                            f"  ❌ [{corp_name}] 로컬 XBRL ZIP 파싱 실패: {parse_err}"
                        )

            # 4. 파싱 성공 시 DB 적재
            if statements:
                logger.info(
                    f"  🎯 [{corp_name}] 로컬 파싱 데이터 수집 성공. 저장소 적재를 가동합니다."
                )
                for fs_type, stmt in statements.items():
                    ...
                    # 수치 찾기 (매출/영익/순익)
                    rev_amount = stmt.find_account_amount(
                        list(self._processing_service.REVENUE_KEYWORDS)
                    )
                    op_amount = stmt.find_account_amount(
                        list(self._processing_service.OP_PROFIT_KEYWORDS)
                    )
                    ni_amount = stmt.find_account_amount(
                        list(self._processing_service.NET_INCOME_KEYWORDS)
                    )
                    ...
                    quarter_rows.append(
                        {
                            ...
                            "매출액": int(rev_amount)
                            if not rev_amount.is_none
                            else None,
                            "영업이익": int(op_amount)
                            if not op_amount.is_none
                            else None,
                            "당기순이익": int(ni_amount)
                            if not ni_amount.is_none
                            else None,
                            "rcept_no": rcept_no,
                            "is_amendment": is_amendment,
                        }
                    ...
                    if quarter_rows:
                        ...
                        self._repository_port.save_partition(
                            dataset_name, corp_code, df
                        )

                company.mark_success(year)
                self._repository_port.save_company_metadata(company)
                logger.info(
                    f"  ✅ [{corp_name}] 로컬 XBRL 기반으로 {year}년 재무 정보 적재 완료"
                )
                return True

            logger.warning(
                f"  ❌ [{corp_name}] XBRL 데이터를 수집하지 못했습니다. API 폴백 없이 다음 실행에서 재시도합니다."
            )
