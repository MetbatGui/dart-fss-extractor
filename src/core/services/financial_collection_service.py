...
class FinancialCollectionService:
    ...
    def collect_and_save(
        ...
        logger.info(
            f"총 {total_companies}개 기업에 대한 수집 작업을 시작합니다. (이어하기 가능, 실패스킵: {skip_failed})"
        )
        ...
        for idx, (name, code) in enumerate(target_companies, 1):
            ...
            if not force_recollect and target_years.issubset(finished_years):
                logger.info(
                    f"[{idx}/{total_companies}] {name} ({code}) - 요청한 기간({start_year}~{end_year}) 데이터가 이미 수집되었거나 실패 기록이 있습니다. 건너뜁니다."
                )
                continue
            ...
            if failed_in_range and not skip_failed:
                logger.info(
                    f"[{idx}/{total_companies}] {name} ({code}) - 실패 이력({failed_in_range}) 재시도 및 누락 데이터 수집 시작..."
                )
            else:
                logger.info(
                    f"[{idx}/{total_companies}] {name} ({code}) - 데이터 수집 시작 (기간: {start_year}~{end_year})..."
                )
            ...
            if company_data:
                ...
                if self._repository_port.exists(dataset_name, code):
                    try:
                        existing_df = self._repository_port.load_partition(
                            dataset_name, code
                        )
                        if not existing_df.empty:
                            ...
                            self._repository_port.save_partition(
                                dataset_name, code, merged_df
                            )
                            logger.info(
                                f"[{idx}/{total_companies}] {name} 기존 데이터와 병합하여 저장 완료."
                            )
                        else:
                            self._repository_port.save_partition(
                                dataset_name, code, new_df
                            )
                            logger.info(f"[{idx}/{total_companies}] {name} 저장 완료.")
                    except Exception as e:
                        logger.error(
                            f"[{idx}/{total_companies}] {name} 병합 저장 중 오류: {e}"
                        )
            ...
            if company.failed_years:
                logger.warning(
                    f"[{idx}/{total_companies}] {name} 일부 실패: {company.failed_years}"
                )
            ...
            if not all_df.empty:
                ...
                if not df_annual.empty:
                    ...
                    final_dfs["매출액_연간"] = df_annual.pivot(
                        index=["기업명"], columns="연도", values="매출액"
                    )
                    final_dfs["영업이익_연간"] = df_annual.pivot(
                        index=["기업명"], columns="연도", values="영업이익"
                    )
                    final_dfs["당기순이익_연간"] = df_annual.pivot(
                        index=["기업명"], columns="연도", values="당기순이익"
                    )
    ...
    def _append_to_list(
        ...
        if metrics.annual_metrics and metrics.annual_metrics.is_valid:
            ...
            if (
                annual.revenue is not None
                or annual.operating_profit is not None
                or annual.net_income is not None
            ):
                data_list.append(
                    {
                        "기업명": name,
