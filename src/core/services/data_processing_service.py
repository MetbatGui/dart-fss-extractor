...
class DataProcessingService:
    ...
    def __init__(self, keywords_config: dict[str, list[str]] | None = None):
    ...
    def extract_metrics(
        self, statement: FinancialStatement, use_cumulative: bool = False
    ) -> FinancialMetrics:
    ...
    def calculate_annual_from_quarters(
        self, metrics_by_quarter: dict[str, FinancialMetrics]
    ) -> FinancialMetrics:
        """수립된 분기 실적 목록을 합산하여 연간 실적을 롤업합니다."""
        temp_metrics = QuarterlyMetrics(
            corp_name="", metrics_by_quarter=metrics_by_quarter
        )
