...
class IncrementalUpdateService:
    ...
    def update_missing_quarters(
        ...
        logger.info(
            f"🚀 증분 업데이트 시작: {target_period} (파일: {file_path}, 강제 업데이트: {force_update})"
        )
        ...
        for sheet_name, df in existing_sheets.items():
            if df.index.duplicated().any():
                ...
                logger.warning(
                    f"⚠️ 시트 '{sheet_name}'에서 중복된 기업명 {dup_count}개 발견. 첫 번째 항목만 유지하고 중복을 제거합니다."
                )
        ...
        if force_update:
            ...
            target_companies = self.find_missing_companies(
                existing_sheets, target_period
            )
        ...
        logger.info(
            f"📋 대상 기업 {len(target_companies)}개 발견: {target_companies[:5]}..."
        )
        ...
        for idx, company_name in enumerate(target_companies, 1):
            ...
            if self._current_api_calls >= self._max_api_calls:
                logger.warning(
                    f"⚠️ API 호출 제한 도달! ({self._current_api_calls}/{self._max_api_calls})"
                )
                logger.info("작업을 중단하고 현재까지 수집된 데이터를 저장합니다.")
                break

            logger.info(
                f"[{idx}/{len(target_companies)}] {company_name} 데이터 수집 중... (API 호출: {self._current_api_calls})"
            )
            ...
            try:
                ...
                # 해당 연도 전체 데이터 수집
                company_data = self._collect_year_for_company(
                    company_name, corp_code, target_year
                )
        ...
        merged_sheets = self.merge_quarterly_data(
            existing_sheets, new_sheets, overwrite=force_update
        )
        ...
        logger.info(
            f"✅ 업데이트 완료! (처리된 기업: {processed_count}개, 총 API 호출: {self._current_api_calls}회)"
        )

    def find_missing_companies(
        self, sheets: dict[str, pd.DataFrame], target_period: str
    ) -> list[str]:
        ...
        if revenue_sheet is None:
            logger.warning(
                "'매출액_분기별' 시트가 없습니다. 모든 기업을 대상으로 간주할 수 없으므로 빈 리스트 반환."
            )
            return []
        ...
        if target_period not in revenue_sheet.columns:
            logger.info(
                f"'{target_period}' 컬럼이 없습니다. 모든 기업을 수집 대상으로 합니다."
            )
    ...
    def _collect_year_for_company(
        self, company_name: str, corp_code: str, year: int
    ) -> list[dict]:
        ...
        for q_num in [1, 2, 3, 4]:
            ...
            stmt = self._financial_port.get_financial_statement(
    ...
    def _convert_to_wide_format(self, data_list: list[dict]) -> dict[str, pd.DataFrame]:
        ...
        sheets = {}
        if not df.empty:
            sheets["매출액_분기별"] = df.pivot(
                index="기업명", columns="기간", values="매출액"
            )
            sheets["영업이익_분기별"] = df.pivot(
                index="기업명", columns="기간", values="영업이익"
            )
            sheets["당기순이익_분기별"] = df.pivot(
                index="기업명", columns="기간", values="당기순이익"
            )
    ...
    def _backup_file(self, file_path: str) -> str:
        ...
        backup_path = file_path_obj.with_name(
            f"{file_path_obj.stem}_backup_{timestamp}.xlsx"
        )
        try:
